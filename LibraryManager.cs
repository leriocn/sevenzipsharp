...
namespace SevenZip
{
    ...
    internal static class SevenZipLibraryManager
    {
        ...
        private static string _libraryFileName = ConfigurationManager.AppSettings["7zLocation"] ??
            Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), Environment.Is64BitProcess ? "x64\\" : "x86\\", "7z.dll");
#endif
#if WINCE 		
        private static string _libraryFileName =
            Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().GetName().CodeBase), Environment.Is64BitProcess ? "x64\\" : "x86\\", "7z.dll");
#endif
        /// <summary>
        ...
        public static void LoadLibrary(object user, Enum format)
        {
            lock (_syncRoot)
            {
                ...
                if (_modulePtr == IntPtr.Zero)
                {
                    ...
                    {
                        throw new SevenZipLibraryException("DLL file does not exist:" + _libraryFileName);
                    }
